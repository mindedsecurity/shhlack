  ...
  function cmpVersions(a, b, sep) {
    ...
    httpget(versionURL,function onend(data){
      ...
      fs.writeFileSync(path.join(shhlackHomePath, "package_new.json"),data);
           ...
           console.error(exc);
       setError({version:local_shhlackPackage.version, new_version: remote_version,error: exc.message, type: "update"});
     }
     },
     function onerror(err){
        console.error(err);       
        setError({version:local_shhlackPackage.version, new_version: remote_version,error: err.message, type: "update"});
     }, 
     ...
     try{
      downloadfile = fs.createWriteStream( path.join(shhlackHomePath, "shhlack_new.js"), {
       ...
       setError({version:local_shhlackPackage.version, new_version: remote_version,error: exc.message, type: "update"});
        ...
        console.error(exc);
      setError({error: exc.message, type: "check_version" });
    }

    },function onerror(err){
       console.error(err);
       setError({error: err.message, type: "check_version" });
    });
